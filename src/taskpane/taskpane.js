...
async function setupTaskpane() {
  ...
  let inputs = ['from', 'to', 'category', 'rate']
  inputs.forEach(inputId => {
    ...
    allCategories.forEach(
        category => {options += '<option value=\'' + category + '\'>' +
                         category + '</option>'})
...
async function loadEvents() {
  ...
  let restUrl = Office.context.mailbox.restUrl +
      '/v2.0/me/events?$select=Start,End,Categories';

  console.log('Fetching: ' + restUrl + ' from ' + window.location)
  return new Promise(
      resolve => {fetch(restUrl, {
                    headers: {
                      ...
                      .catch(function(error) {
                        console.error(error)
                        setStatus('Got error fetching events: ' + error)
                      })})
  // $.ajax({url: restUrl, headers: {'Authorization': 'Bearer ' + accessToken}})
  //     .done(function(res) {
  //       console.log(res)
  //       return res
  //     })
...
async function login() {
  return new Promise(
      resolve => Office.context.mailbox.getCallbackTokenAsync(
